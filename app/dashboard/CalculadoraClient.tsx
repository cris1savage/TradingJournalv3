...
const inp: React.CSSProperties = { background: G.card2, border: `1px solid ${G.border}`, borderRadius: 8, padding: '10px 13px', color: G.text, fontFamily: 'Outfit,sans-serif', fontSize: 14, width: '100%', outline: 'none' };
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
      ...
      <div style={{ background: G.card, border: `1px solid ${G.border}`, borderRadius: 14, padding: 20 }}>
        <div style={{ fontSize: 13, fontWeight: 700, fontFamily: 'Outfit,sans-serif', marginBottom: 16 }}>⚙️ Parámetros del Trade</div>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
            {Object.entries(PARES).map(([k, v]) => (
              <button key={k} onClick={() => setPar(k)} style={{ padding: '6px 12px', borderRadius: 7, border: `1px solid ${par===k?G.accent:G.border}`, background: par===k?`${G.accent}20`:'transparent', color: par===k?G.accent:G.muted, fontSize: 11, cursor: 'pointer', fontFamily: 'monospace', fontWeight: par===k?700:400 }}>{k}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <div style={{ background: G.card2, border: `1px solid ${G.border}`, borderRadius: 8, padding: '10px 13px', fontFamily: 'monospace', fontSize: 14, color: G.cyan }}>{capital.toFixed(2)}€</div>
        </div>
        ...
        <div style={{ marginBottom: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
            ...
            <span style={{ fontFamily: 'monospace', fontSize: 11, color: G.accent }}>{riesgoN}% = {riesroEur.toFixed(2)}€</span>
          </div>
          <div style={{ display: 'flex', gap: 5, marginBottom: 6 }}>
            {['0.5','1','1.5','2','3'].map(v => (
              <button key={v} onClick={() => setRiesgoPct(v)} style={{ flex: 1, padding: '7px 4px', borderRadius: 7, border: `1px solid ${riesgoPct===v?G.accent:G.border}`, background: riesgoPct===v?`${G.accent}20`:'transparent', color: riesgoPct===v?G.accent:G.muted, fontSize: 11, cursor: 'pointer', fontFamily: 'monospace' }}>{v}%</button>
            ))}
          </div>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
            <button onClick={() => setDir('buy')} style={{ padding: '9px', borderRadius: 8, border: `1px solid ${dir==='buy'?G.green:G.border}`, background: dir==='buy'?`${G.green}15`:'transparent', color: dir==='buy'?G.green:G.muted, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'Outfit,sans-serif' }}>▲ LONG</button>
            <button onClick={() => setDir('sell')} style={{ padding: '9px', borderRadius: 8, border: `1px solid ${dir==='sell'?G.red:G.border}`, background: dir==='sell'?`${G.red}15`:'transparent', color: dir==='sell'?G.red:G.muted, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'Outfit,sans-serif' }}>▼ SHORT</button>
          </div>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ background: lote > 0 ? `${G.accent}10` : G.card, border: `2px solid ${lote > 0 ? G.accent : G.border}`, borderRadius: 14, padding: 22, textAlign: 'center', boxShadow: lote > 0 ? `0 0 30px ${G.accent}20` : 'none' }}>
          <div style={{ fontFamily: 'monospace', fontSize: 9, color: G.muted, letterSpacing: '0.2em', marginBottom: 10, textTransform: 'uppercase' }}>TAMAÑO DE POSICIÓN</div>
          <div style={{ fontFamily: 'Outfit,sans-serif', fontSize: 52, fontWeight: 900, color: lote > 0 ? G.accent : G.muted, lineHeight: 1, marginBottom: 6 }}>{loteDisplay}</div>
          <div style={{ fontFamily: 'monospace', fontSize: 12, color: G.muted }}>lotes</div>
          {lote > 0 && loteStandard !== lote && (
            <div style={{ marginTop: 10, padding: '7px 14px', background: `${G.gold}15`, border: `1px solid ${G.gold}30`, borderRadius: 8, display: 'inline-block', fontSize: 12, color: G.gold, fontFamily: 'Outfit,sans-serif' }}>
        ...
        {rr > 0 && (
          <div style={{ background: rr >= 2 ? `${G.green}10` : rr >= 1.5 ? `${G.gold}10` : `${G.red}10`, border: `1px solid ${rr >= 2 ? G.green : rr >= 1.5 ? G.gold : G.red}30`, borderRadius: 12, padding: '14px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontFamily: 'monospace', fontSize: 9, color: G.muted, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 4 }}>RATIO RIESGO:BENEFICIO</div>
              <div style={{ fontFamily: 'Outfit,sans-serif', fontSize: 24, fontWeight: 800, color: rr >= 2 ? G.green : rr >= 1.5 ? G.gold : G.red }}>1 : {rr.toFixed(2)}</div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 11, color: rr >= 2 ? G.green : rr >= 1.5 ? G.gold : G.red, fontFamily: 'Outfit,sans-serif', fontWeight: 600 }}>
        ...
        {/* Warning */}
        {lote > 0.1 && riesgoN > 2 && (
          <div style={{ background: `${G.red}10`, border: `1px solid ${G.red}30`, borderRadius: 10, padding: '11px 14px', fontSize: 12, color: G.red, fontFamily: 'Outfit,sans-serif' }}>
        ...
        {/* Copy button */}
        {lote > 0 && (
          <button onClick={() => { navigator.clipboard?.writeText(`${par} ${dir === 'buy' ? 'BUY' : 'SELL'} | Lote: ${loteDisplay} | SL: ${sl} | TP: ${tp || '—'} | Riesgo: ${riesroEur.toFixed(2)}€`); }} style={{ padding: '11px', background: `${G.accent}15`, border: `1px solid ${G.accent}40`, borderRadius: 10, color: G.accent, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'Outfit,sans-serif' }}>
